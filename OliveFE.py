...
class COCO_dataset:
    ...
    def __init__(self, cfg, args):
        ...
        self.aug_category = aug_category([
            'stem', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 
...
class vis_tool:
    ...
    def __init__(self):
        ...
        self.th_button = Button(
            self.window, text='Save counts', height=1, command=self.change_threshold)
    ...
    def change_threshold(self, event=None):
        try:
            ...
            self.window.after(2000,  self.save_all_counts)
    ...
    def save_all_counts(self, event=None):
        #print(self.img_list)
        counts = np.zeros(len(self.img_list))
        for i, name in enumerate(self.img_list):
            img = self.data_info.get_img_by_name(name)
            img = np.asarray(img)
            result = inference_detector(self.model, img)
            scores = self.get_dets(result)[0][:,4]
            counts[i] = len(scores[scores >= self.threshold])
        data = {'Image': self.img_list,
                'Count': counts
        }
        #print(data)
        df = pd.DataFrame(data, columns= ['Image', 'Count'])
        df.to_csv(r'output/counts_threshold%f.csv' % self.threshold, index = False, header=True)
        print('Success! All counts saved.')
    ...
    def change_img(self, event=None):
        ...
        self.window.title("Olive Flower Enumerator" + '   ' +'DATASET : ' + self.data_info.dataset + '   ' + name)
